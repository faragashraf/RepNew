...
Public Class TikDetails
    ...
    Private Sub TikDetails_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Fn.SelctSerchTxt(TxtDetails, "تعديل : بواسطة")
    End Sub
    Private Sub BtnAddEdt_Click(sender As Object, e As EventArgs) Handles BtnAddEdt.Click
        ...
        If Trim(TxtDetailsAdd.Text).Length > 0 Then
            If Fn.InsUpdate("update Tickets set TkDetails = '" & TxtDetails.Text & vbCrLf & "تعديل : بواسطة  " & Usr.PUsrRlNm & " في " & Fn.ServrTime() & " من خلال IP : " & OsIP() & vbCrLf & TxtDetailsAdd.Text & "' where TkSQL = " & StruGrdTk.Sql, "000&H") = Nothing Then
                TxtDetails.Text &= vbCrLf & "تعديل : بواسطة  " & Usr.PUsrRlNm & " في " & Fn.ServrTime() & " من خلال IP : " & OsIP() & vbCrLf & TxtDetailsAdd.Text
                Fn.SelctSerchTxt(TxtDetails, "تعديل : بواسطة")
    ...
    Private Sub BtnClos_Click(sender As Object, e As EventArgs) Handles BtnClos.Click
        ...
        If Rslt = DialogResult.Yes Then
            ...
            If Fn.InsTrans("update Tickets set TkDtClose = (Select GetDate())" & ", TkDuration = " & Fn.CalDate(StruGrdTk.DtStrt, Nw, "1036&H") & ", TkClsStatus = 1" & ", TkFolw = 1" & " where (TkSQL = " & StruGrdTk.Sql & ");",
                ...
                TxtDetailsAdd.TextAlign = HorizontalAlignment.Center
                TxtDetailsAdd.Font = New Font("Times New Roman", 16, FontStyle.Regular)
                MsgInf("تم إغلاق الشكوى رقم " & StruGrdTk.TkId & " في عدد " & Fn.CalDate(StruGrdTk.DtStrt, CStr(Nw), "1036&H") & " يوم عمل")
            Else
