Public Class Form1Copy
    ...
    Private Sub Adjust_()
        ...
        For f = 0 To Me.Controls.Count - 1
            Me.Controls(f).Size = New Point((Screen.PrimaryScreen.Bounds.Width / Val(ToolStripTextBox1.Text)), (Screen.PrimaryScreen.Bounds.Height / Val(ToolStripTextBox1.Text)))
            If f > 0 Then
                If ToolStripTextBox1.Text = ItemPerLine Then
                    Y_ += Me.Controls(f).Size.Height
                    X_ += 0
                    ItemPerLine = 0
                Else
                    X_ += Me.Controls(f).Size.Width
                End If
            End If
            Me.Controls(f).Location = New Point(X_, Y_)
            ItemPerLine += 1
        Next
    End Sub
